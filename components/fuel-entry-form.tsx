    ...
    try {
      ...
      const response = await fetch("/api/fuel-entries", {
        ...
        body: JSON.stringify({
          ...
          work_distance_km: formData.work_distance_km ? Number.parseInt(formData.work_distance_km) : null,
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {formData.is_work_travel && (
            <div className="space-y-2 rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-900 dark:bg-blue-950">
              <Label htmlFor="work_distance_km" className="text-blue-900 dark:text-blue-100">
                Work Distance (km) - Optional
              </Label>
              <Input
                id="work_distance_km"
                name="work_distance_km"
                type="number"
                placeholder="e.g., 150"
                value={formData.work_distance_km}
                onChange={handleInputChange}
                className="bg-background"
              />
              <p className="text-xs text-blue-700 dark:text-blue-300">
                Specify the portion of this trip that was for work purposes (must be less than or equal to total
                distance traveled)
              </p>
            </div>
          )}
